    ...
    protected void autoMigrate() {
        ...
        if (config.migrateRegionsToUuid) {
            ...
            try {
                ...
                WorldGuard.logger.info("Oblasti uložené po migraci UUID! To se nebude opakovat, pokud " +
                        "nezměníte příslušnou možnost konfigurace v konfiguraci WorldGuard.");

                config.disableUuidMigration();
            } catch (MigrationException e) {
                WorldGuard.logger.log(Level.WARNING, "Migraci se nepodařilo provést", e);
            }
        }
    }
