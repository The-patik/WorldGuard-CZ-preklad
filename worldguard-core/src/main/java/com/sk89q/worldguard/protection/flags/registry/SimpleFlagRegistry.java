...
public class SimpleFlagRegistry implements FlagRegistry {
    ...
    public void register(Flag<?> flag) throws FlagConflictException {
        synchronized (lock) {
            if (initialized) {
                throw new IllegalStateException("V tuto chvíli nelze registrovat nové značky");
            }
    ...
    private Flag<?> forceRegister(Flag<?> flag) throws FlagConflictException {
        ...
        synchronized (lock) {
            ...
            if (flags.containsKey(name)) {
                throw new FlagConflictException("Značky pod tímto jménem již existuje: " + name);
            }
            ...
            if (flag != null) {
                try {
                    ...
                    if (unmarshalled != null) {
                        ...
                        log.warning("Nepodařilo se analyzovat značku '" + flag.getName() + "' s hodnotou '" + entry.getValue() + "'");
                    }
                } catch (Throwable e) {
                    log.log(Level.WARNING, "Nepodařilo se uvolnit hodnotu značky pro " + flag, e);
                }
            }
        }
