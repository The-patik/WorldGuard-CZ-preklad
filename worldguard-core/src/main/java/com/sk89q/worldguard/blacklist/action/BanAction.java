...
public class BanAction implements Action {
    ...
    public ActionResult apply(BlacklistEvent event, boolean silent, boolean repeating, boolean forceRepeat) {
        ...
        if (event.getPlayer() != null) {
            ...
            if (message != null) {
                event.getPlayer().ban("Zabanován: " + String.format(message, event.getTarget().getFriendlyName()));
            } else {
                event.getPlayer().ban("Zabanován: Nemůžeš " + event.getDescription() + " " + event.getTarget().getFriendlyName());
            }
        }
